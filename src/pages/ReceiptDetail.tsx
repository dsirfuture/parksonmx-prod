...
function parseReceiptListPayload(payload: any): ReceiptRow[] {
  if (!payload) return [];
  const cands = [payload, payload?.data, payload?.data?.data, payload?.data?.receipts, payload?.receipts, payload?.rows, payload?.data?.rows];
...
export default function ReceiptDetail() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!k) return items;
    return items.filter((it) => `${it.sku || ""} ${it.barcode || ""} ${it.name_zh || ""} ${it.name_es || ""}`.toLowerCase().includes(k));
  }, [items, q]);
  ...
  function statusText(it: ReceiptItemRow) {
    ...
    const ev = Array.isArray(it?.evidence_photo_urls) ? it.evidence_photo_urls.length : toInt(it?.evidence_photo_count ?? it?.evidence_count);
  ...
  return (
    <div className="min-h-screen bg-[#F4F6FA] flex flex-col">
      <Header title="验货单详情" onBack={goBack} />
      ...
      {/* SKU 证据分享弹窗（保持你原有：WhatsApp 直接跳转，WeChat 复制） */}
      {shareOpen && shareItem ? (
