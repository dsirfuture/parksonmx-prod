...
export default function AdminPcScan() {
  ...
  async function submitScan(raw: string, mode: "good" | "damaged") {
    ...
    if (last.code === `${mode}:${v}` && now - last.ts < 300) return; // PC按钮点击防抖更短
      ...
      <main className="flex-1 w-full max-w-[1400px] mx-auto px-6 pt-4 pb-6 space-y-3 select-text">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4">
          ...
          <div className="mt-4 overflow-auto rounded-2xl border border-slate-200 select-text">
            {/* ✅ 增加“破损+1”按钮列（放在破损数旁边） */}
            <table className="w-full min-w-[1380px] bg-white select-text">
              <thead className="bg-[#F4F6FA]">
                <tr className="text-[12px] text-slate-600 font-extrabold select-text">
                  ...
                  <th className="text-center p-3">{L("破损+1", "+1")}</th>
              ...
              <tbody>
                ...
                {filteredItems.length === 0 ? (
                  <tr>
                    <td className="p-6 text-center text-[12px] text-slate-400 font-semibold select-text" colSpan={10}>
                      {L("暂无数据", "Sin datos")}
                    </td>
                  </tr>
                ) : null}
